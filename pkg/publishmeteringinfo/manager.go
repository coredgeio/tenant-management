...
func (m *PublishMeteringInfoManager) sendMeteringData(domain string, event *events.Event) {
	if event.Event.Resource == events.BMSResource {
		...
		case events.BMSAllocateEvent:
			...
			reqBody, _ = m.prv.BuildMeteringInfo(events.BMSAllocateEvent)
		case events.BMSReleaseEvent:
			// make an API call to send metering data
			reqBody, _ = m.prv.BuildMeteringInfo(events.BMSReleaseEvent)
		default:
...
func CreatePublishMeteringInfoManager(prvdr provider.Provider) *PublishMeteringInfoManager {
	...
	manager := &PublishMeteringInfoManager{
		...
		baseUrl:             cfg.GetPublishMeteringInfoBaseUrl(),
		httpMethod:          cfg.GetPublishMeteringInfoHttpMethod(),
		contentTypeHeader:   cfg.GetPublishMeteringInfoContentType(),
		authorizationHeader: cfg.GetPublishMeteringInfoAuthorization(),
		apiKey:              cfg.GetPublishMeteringInfoApiKey(),
		mu:                  sync.Mutex{},
		prv:                 prvdr,
	}
